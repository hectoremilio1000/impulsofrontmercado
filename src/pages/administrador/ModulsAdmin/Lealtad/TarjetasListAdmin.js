...
export default function TarjetasList({ programId }) {
  ...
  const fetchCards = async () => {
    setLoading(true);
    try {
      let url = `${apiUrl}/loyalty/cards?user_id=${auth.user.id}`;
      if (programId) {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 130,
      render: (_, record) => {
        // Accedemos a record.program.company
        if (record.program && record.program.company) {
          return record.program.company.name;
        } else {
          return "Sin empresa";
        }
      },
  ...
  return (
    <div>
      Recuerda tu contraseña es tu número de usuario: <b>{auth.user.id}</b>
